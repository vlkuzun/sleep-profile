"""Central sleep-stage color palette for all visualisation scripts.

Import ``get_stage_color`` or the constant mappings to ensure consistent
color choices whenever plotting Wake, NREM, and REM stages.
"""
...
def get_stage_palette(labels: list[str] | None = None) -> list[str]:
    """Return colors for the provided labels, defaulting to Wake/NREM/REM order."""

    if labels is None:
        labels = ["Wake", "NREM", "REM"]
    return [get_stage_color(label) for label in labels]
